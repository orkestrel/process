...
const PACKAGE_NAME = '@orkestrel/process'
...
await new GuideCommand({
	...
	const { holds, isRecord, parseJSON } = await import('@orkestrel/contract')
	const {
		computeSymbolKey,
		createGuide,
		createSource,
	...
	const { requireValue, waitForCondition, waitForDelay } = await import('@orkestrel/test')
	const {
	...
	it('manifest lists at least one guide', () => {
		...
		expect(own.entry.source).toEqual(['src/core', 'src/server'])
	})
	...
	describe('public package faces', () => {
		...
		it('derives the exact package export keys from the same face map', () => {
			const exports: unknown = Object.getOwnPropertyDescriptor(manifest, 'exports')?.value
			if (typeof exports !== 'object' || exports === null) {
				throw new Error('The package manifest declares no object exports')
			}
			const expected = FACES.map((face) =>
				face.specifier === PACKAGE_NAME ? '.' : `.${face.specifier.slice(PACKAGE_NAME.length)}`,
			)
	...
	describe('flagship fences', () => {
		...
		it('names every constant literal in the Summary its Constants table prints', () => {
			...
			expect(records.map((row) => row.name).sort()).toEqual(Object.keys(CONSTANTS).sort())
		...
		it('binds the command and supervision host qualifications', () => {
			...
			expect(types).toContain(
				'A consumer must call `stop` or `destroy` during an orderly shutdown.',
			)
		})
	...
	const EXAMPLES = Object.freeze([
		...
		{
			name: 'captureChunk',
			value: captureChunk(Buffer.from('hello'), 3),
			claim: Buffer.from('hel'),
		},
