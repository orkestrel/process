...
| API                     | Kind      | Summary                                                                                                                                                                                               |
| ----------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ProcessChildInterface` | interface | The child boundary the termination helpers drive — `pid`, `exitCode`, `signalCode`, `kill`, `once`, `off`.                                                                                            |
| `SupervisorFace`        | interface | The face callbacks one supervised child reports to — `chunk`, `fault`, `relieve`, `close`, `terminal`, `teardown`. The engine that takes them stays out of the barrel; see [Vocabulary](#vocabulary). |
