...
**Ruled: replace the record with a `Capture` sub-entity.** `AGENTS.md` § Design laws names this
remedy — where a shape does not fit, extract a sub-entity. A `Capture` holding `#delivered` and
...
Declare it in `src/server/types.ts` before implementing it. `retainChunk` folds into the entity's
method: an exported free function that mutates a caller's accumulator has no remaining reason to
exist once the accumulator owns the behaviour. Update `src/server/helpers.ts` and its callers, the
guide's Capture-helpers rows, and every test.

`guides/process.md`'s row for this type leads with a mutability adjective and an implementation
rationale and never names its members, where every neighbouring Summary cell names the contract's
members. Restate it in the table's established form.

If you conclude the sub-entity is wrong, stop and report with your reasoning. Do not implement a
third shape.
...
- **Owned:** `src/server/types.ts`, `src/server/helpers.ts`, `src/server/index.ts`,
  `guides/process.md`, `tests/guides.test.ts`, `tests/distribution.test.ts`, and every test file
  under `tests/src/`.
