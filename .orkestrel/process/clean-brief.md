# CLEAN-PROCESS — delete every count from this package's prose
...
Delete every count and every positional reference from `/workspace/process`'s prose, so no sentence anywhere in
...
The rule now reads, in `/workspace/process/AGENTS.md` § Writing:
...
Read before acting: `/workspace/process/AGENTS.md` § Writing, `/workspace/process/.claude/rules/writing.md`, and
`/workspace/process/.claude/rules/documentation.md`.

Host: Linux container, bash. `/workspace/process` is a clean checkout. `npx scaffold audit` resolves normally here.
...
Owned files: every prose surface in `/workspace/process` — `AGENTS.md`, `CLAUDE.md`, `README.md`, everything
under `guides/`, `.agents/`, `.claude/`, and every TSDoc block and code comment under `src/`, `app/`,
`tests/`, `configs/`, and `scripts/`.

`AGENTS.md`, `CLAUDE.md`, `.agents/`, and `.claude/` in this package are vendored copies of `@orkestrel/scaffold`'s. The count rule was already landed in scaffold and its bytes will arrive here through `repair` after scaffold publishes, so **do not edit a vendored file**: leave `AGENTS.md`, `CLAUDE.md`, `.agents/`, `.claude/rules/`, `.claude/agents/`, and `.claude/skills/` exactly as they are. Everything else in this package is yours — `README.md`, `guides/`, `src/`, `tests/`, `configs/`, and `scripts/`. Note that `guides/` holds vendored mirrors of other packages' guides — `guides/test.md`, `guides/scaffold.md`, `guides/guide.md`, `guides/contract.md`, `guides/emitter.md`. Those are fetched bytes rather than authored prose and are off-limits too. `guides/process.md` is this package's own and is yours.
